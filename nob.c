...
int main(int argc, char **argv)
{
    ...
    if (!compile_test_exe("test", TEST_SRC_DIR "test.c")) return 1;
    if (!compile_test_exe("test_create", TEST_SRC_DIR "test_create.c")) return 1;
    if (!compile_test_exe("test_targz", TEST_SRC_DIR "test_targz.c")) return 1;
    if (!compile_test_exe("test_zip", TEST_SRC_DIR "test_zip.c")) return 1;
    
    // Build example
    if (do_example || argc == 0) {
        if (!compile_example_exe("extract_src", EXAMPLE_DIR "extract_src.c")) return 1;
    }
